...
for title in fastwalshtransform mergesort scalarprod scan-long scan-short sortingnetworks-bitonic sortingnetworks-oddeven transpose vectoradd; do
    echo "Running: python schedule/schedule.py --collect etc/cuda-sdk/${title}.yaml --no-profile --no-trace"
    python schedule/schedule.py --collect etc/cuda-sdk/${title}.yaml --no-profile --no-trace || true
done